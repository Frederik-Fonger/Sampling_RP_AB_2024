...
class CminSampler:
    ...
    def load_list(self, lst, index=None):
        ...
        # Expected occurrence reduction reduces the number of operations needed (see paper)
        self.eo_counts = np.floor(
            ((self.multiplicity / self.multiplicity.sum())) * self.p)
        self.variants = OrderedDict(
            {x: int(y - self.eo_counts[i]) for i, (x, y) in enumerate(self.variants.items())})
    ...
    def sample(self, type='auto', output='case'):
        ...
        if output == 'seq':
            ...
            for i, x in enumerate(sampler):
                case_output.extend(np.random.choice(self.variant_to_id[i], size=int(
                    x), replace=x > len(self.variant_to_id[i])).tolist())
    ...
    def samplingWithDM(self):
        ...
        while len(output_count) != self.p-self.eo_counts.sum():
            ...
            if subset_m.shape[1] > self.capacity:
                ...
                nsmallest_sum_index = np.argpartition(subset_m, self.capacity, axis=1)[
                    :, :self.capacity]
                n_smallest_sum_value = np.take_along_axis(
                    subset_m, nsmallest_sum_index, 1)
    ...
    def samplingWithEucl(self):
        ...
        original_seq_index = np.arange(
            self.multiplicity.shape[0]).repeat(self.multiplicity)
    ...
    def buildDistanceMatrix(self):
        ...
        def d(x1, x2): return editdistance.eval(
            x1, x2) / max([len(x1), len(x2)])
    ...
    def buildSignature(self):
        cv = CountVectorizer(ngram_range=(
            1, 2), tokenizer=lambda doc: doc, lowercase=False, max_features=1024)
        data = cv.fit_transform([['$$START$$']+list(x)+['$$END$$']
                                for x in list(self.variants.keys())])
        data = TruncatedSVD(
            min(64, int(data.shape[1]/2)+1)).fit_transform(data).astype(np.float32)
        return data
